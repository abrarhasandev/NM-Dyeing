...
export async function GET(req, { params }) {
  ...
  if (__authError) return __authError;
    ...
    try {
        await connectDB();
        const { dyeingId, snapshotId } = await params;

        if (!mongoose.Types.ObjectId.isValid(dyeingId) || !mongoose.Types.ObjectId.isValid(snapshotId)) {
            return NextResponse.json({ success: false, message: "Invalid ID" }, { status: 400 });
        }

        const snapshot = await LedgerSnapshot.findOne({
            _id: new mongoose.Types.ObjectId(snapshotId),
            entityId: new mongoose.Types.ObjectId(dyeingId),
            entityType: "dyeing",
        });
