...
export async function GET(req, props) {
  ...
  if (__authError) return __authError;
    ...
    try {
        await connectDB();
        const { customerId, snapshotId } = await params;

        if (!mongoose.Types.ObjectId.isValid(customerId) || !mongoose.Types.ObjectId.isValid(snapshotId)) {
            return NextResponse.json({ success: false, message: "Invalid ID" }, { status: 400 });
        }

        const snapshot = await LedgerSnapshot.findOne({
            _id: new mongoose.Types.ObjectId(snapshotId),
            entityId: new mongoose.Types.ObjectId(customerId),
            entityType: "customer",
        });
