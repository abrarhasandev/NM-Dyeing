...
export async function POST(req, props) {
  ...
  if (__authError) return __authError;
    ...
    try {
        ...
        if (!finalCustomer) return NextResponse.json({ success: false, message: "Customer not found" }, { status: 404 });

        // Either this ledger has new billing/payment activity, OR we are just closing it to archive the `initialAmount`
        const hasInitial = (finalCustomer.initialCharge > 0 || finalCustomer.initialPayment > 0);
        ...
        const initialCharge = finalCustomer.initialCharge || 0;
        const initialPayment = finalCustomer.initialPayment || 0;
        const initialDate = finalCustomer.initialDate || null;
