...
const Page = () => {
  ...
  const fetchTypes = async () => {
    try {
      const res = await fetch("/api/menu/sill-name");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editingId) {
        res = await fetch(`/api/menu/sill-name/${editingId}`, {
          method: "PUT",
        ...
        res = await fetch("/api/menu/sill-name", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name }),
        });
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/menu/sill-name/${deleteId}`, {
        method: "DELETE",
      });
