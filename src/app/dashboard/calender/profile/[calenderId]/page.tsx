...
import InitialAmountModal from "@/components/calender/InitialAmountModal";
import CloseModal from "@/components/calender/CloseModal";
...
export default function CalenderProfileLedger({ params }) {
  ...
  const fetchCurrentLedger = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/calender/ledger/${calenderId}?_t=${Date.now()}`
      );
  ...
  const fetchSnapshots = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/calender/ledger/${calenderId}/snapshots?_t=${Date.now()}`
      );
      const result = await res.json();
  ...
  const loadSnapshot = useCallback(
    async (snapshotId) => {
      ...
      try {
        const res = await fetch(
          `/api/calender/ledger/${calenderId}/snapshots/${snapshotId}`
        );
        const result = await res.json();
        if (result.success)
  ...
  const handleSaveSelected = async (title, saveMode) => {
    ...
    if (saveMode === "ledger") {
      try {
        const invRes = await fetch(
          `/api/calender/ledger/${calenderId}/saved-invoices?view=${selectedView}&_t=${Date.now()}`
        );
        ...
        if (savedInvoices.length > 0) {
          ...
          if (lastInvoice.records && lastInvoice.records.length > 0) {
            ...
            if (recordWithBalance) {
              ...
              foundBalance = true;
              prevInvoiceDesc = ` (${lastInvoice.invoiceNumber} — ${
                lastInvoice.title || "Invoice"
              })`;
    ...
    try {
      const res = await fetch(
        `/api/calender/ledger/${calenderId}/saved-invoices`,
        {
          method: "POST",
  ...
  return (
    <>
      ...
      <div className="mt-12 md:mt-8 lg:mt-1 max-w-6xl mx-auto p-3 sm:p-6 min-h-screen">
        <button
          onClick={() => router.back()}
          className="flex cursor-pointer items-center gap-2 bg-blue-100 px-2 py-1 rounded text-gray-600 hover:text-blue-600 font-bold text-sm mb-4 print:hidden"
        >
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto print:hidden flex-wrap">
                    ...
                    {dropdownOpen && (
                      <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-100 z-40 py-1 overflow-hidden">
                        <button
                          ...
                          className={`w-full cursor-pointer text-left px-4 py-2.5 text-xs font-bold hover:bg-blue-50 flex items-center gap-2 transition ${
                            isCurrentView
                        ...
                        {snapshots.length > 0 && (
                          <>
                            ...
                            <p className="px-4 py-1 text-[9px] text-gray-400 font-black uppercase tracking-widest">
                              Closed Ledgers
                            </p>
                            {snapshots.map((snap) => (
                              <button
                                ...
                                className={`w-full cursor-pointer text-left px-4 py-2.5 hover:bg-gray-50 transition flex items-start gap-2 ${
                ...
                {activeTab === "ledger" && isCurrentView && (
                  <button
                    ...
                    className="cursor-pointer flex items-center gap-2 bg-indigo-500 text-white px-4 py-2.5 rounded-xl text-xs font-black hover:bg-indigo-600 transition whitespace-nowrap"
                  >
                    <Pencil size={10} />{" "}
