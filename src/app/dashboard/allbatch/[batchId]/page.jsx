...
export default function BatchEditPage() {
  ...
  // Extras
  const [editingExtraRow, setEditingExtraRow] = useState(null);
  ...
  const handleDeleteRow = (rollNo) => {
    if (!confirm(`Roll No: ${rollNo} Delete?`)) return;
    const removedRow = embeddedBatch.rows.find((row) => row.rollNo === rollNo);
    const updatedRows = embeddedBatch.rows.filter((row) => row.rollNo !== rollNo);
    setEmbeddedBatch({ ...embeddedBatch, rows: updatedRows });
    if (removedRow) setAvailableRows((prev) => [...prev, removedRow]);
  };

  const handleAddRow = (rollNo) => {
    const rowToAdd = availableRows.find((r) => r.rollNo === rollNo);
    if (!rowToAdd) return;
    setEmbeddedBatch({ ...embeddedBatch, rows: [...embeddedBatch.rows, rowToAdd] });
  ...
  const handleIndexChange = (e, rowRollNo) => {
    ...
    const updatedRows = embeddedBatch.rows.map((row) =>
      row.rollNo === rowRollNo ? { ...row, idx: value > 0 ? [value] : [] } : row
    );
    setEmbeddedBatch({ ...embeddedBatch, rows: updatedRows });
  };

  // Extra input handlers
  const handleAddExtra = (rollNo) => {
    const updatedRows = embeddedBatch.rows.map((row) =>
      row.rollNo === rollNo
        ? { ...row, extraInputs: [...(row.extraInputs || []), ""] }
        : row
    );
    setEmbeddedBatch({ ...embeddedBatch, rows: updatedRows });
    setEditingExtraRow(rollNo);
  };

  const handleExtraChange = (rollNo, index, value) => {
    const updatedRows = embeddedBatch.rows.map((row) => {
      if (row.rollNo === rollNo) {
        const extras = [...row.extraInputs];
        extras[index] = value;
        return { ...row, extraInputs: extras };
      }
      return row;
    });
    setEmbeddedBatch({ ...embeddedBatch, rows: updatedRows });
  ...
  const handleDeleteExtra = (rollNo, index) => {
    const updatedRows = embeddedBatch.rows.map((row) => {
      if (row.rollNo === rollNo) {
        const extras = row.extraInputs.filter((_, i) => i !== index);
        return { ...row, extraInputs: extras };
      }
      return row;
    });
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="bg-white shadow-md rounded-lg border p-5">
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-gray-800">{embeddedBatch.batchName}</h2>
          ...
          <table className="min-w-full border text-sm text-gray-700">
            ...
            <tbody>
              {embeddedBatch.rows.length > 0 ? (
                embeddedBatch.rows.map((row) => (
                  <tr key={row.rollNo} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2 text-center">
                      {editingExtraRow === row.rollNo ? (
                        <div className="flex flex-col gap-2 items-center">
                          {row.extraInputs?.map((val, i) => (
                            <div key={i} className="flex gap-1 items-center">
                              <input
                                type="text"
                                value={val}
                                onChange={(e) => handleExtraChange(row.rollNo, i, e.target.value)}
                                className="border rounded px-2 py-1 w-28 text-sm"
                              />
                              <button onClick={() => handleDeleteExtra(row.rollNo, i)} className="text-red-500">❌</button>
                            </div>
                          ))}
                          <button onClick={() => handleAddExtra(row.rollNo)} className="text-blue-600 text-sm">➕ Add More</button>
                          <button onClick={() => setEditingExtraRow(null)} className="text-green-600 text-sm font-medium">✔ Done</button>
                        </div>
                      ) : row.extraInputs && row.extraInputs.length > 0 ? (
                        <div className="flex flex-wrap gap-1 justify-center items-center">
                          {row.extraInputs.map((ex, i) => (
                            <span key={i} className="px-2 py-0.5 bg-gray-100 rounded text-xs">{ex}</span>
                          ))}
                          <button onClick={() => setEditingExtraRow(row.rollNo)} className="text-blue-600">✏️</button>
                        </div>
                      ) : (
                        <button onClick={() => handleAddExtra(row.rollNo)} className="text-blue-600 text-lg">➕</button>
                      )}
                    </td>
                    <td className="p-2 text-center">
                      <button onClick={() => handleDeleteRow(row.rollNo)} className="text-red-500 hover:text-red-700">
                ...
                <tr>
                  <td colSpan={5} className="text-center text-gray-400 p-4 italic">No rows currently in this batch</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        {/* Add Row Section */}
        <div className="mb-4">
          <button onClick={() => setAddingRow(!addingRow)} className="flex items-center bg-blue-600 text-white px-4 py-2 rounded-md cursor-pointer hover:bg-blue-700">
          ...
          {addingRow && (
            <div className="mt-3 border border-gray-200 p-3 rounded-md bg-gray-50 max-h-56 overflow-y-auto">
              {availableRows.length > 0 ? (
                availableRows.map((r) => (
                  <div key={r.rollNo} className="flex justify-between items-center py-1 border-b last:border-0 text-sm">
                    <span>Roll No: <span className="font-semibold">{r.rollNo}</span> | Goj: {r.goj}</span>
                    <button onClick={() => handleAddRow(r.rollNo)} className="text-green-600 font-medium">Add</button>
                  </div>
                ))
              ) : (
                <p className="text-gray-400 text-sm">No available rows to add</p>
              )}
            </div>
          )}
        </div>
