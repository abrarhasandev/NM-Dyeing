...
const CreateCustomerPage = () => {
  ...
  const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm<CustomerFormValues>({
  ...
  const onSubmit = async (data: CustomerFormValues) => {
    ...
    try {
      // The API uses the new structured data
    ...
    <div className="min-h-screen bg-background text-foreground p-4 md:p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Create Customer</h1>
          <p className="text-muted-foreground mt-1">Add a new company or individual customer</p>
        </motion.div>
            ...
            {addressFields.map((field, index) => (
              <div key={field.id} className="bg-accent/30 p-4 rounded-lg border border-border/50 relative mb-4">
                ...
                <AddressSelector
                  ...
                  addressData={watchAddresses[index]}
