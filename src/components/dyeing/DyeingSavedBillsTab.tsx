...
import { ChevronDown, ChevronUp, Plus, Printer, Trash2, X } from "lucide-react";

function DyeingSavedBillsTab({ dyeingId, selectedView, availableRows, onInvoiceUpdated, companyAddress }) {
    ...
    }, [dyeingId, selectedView]);

    async function fetchInvoices() {
        setLoading(true);
        try {
            const res = await fetch(`/api/dyeings/ledger/${dyeingId}/saved-invoices?view=${selectedView}&_t=${Date.now()}`);
    ...
    const handleRemoveRecord = async (invoiceId, record) => {
        ...
        try {
            const res = await fetch(`/api/dyeings/ledger/${dyeingId}/saved-invoices/${invoiceId}`, {
                method: "PATCH",
    ...
    const handleAppend = async () => {
        ...
        try {
            const res = await fetch(`/api/dyeings/ledger/${dyeingId}/saved-invoices/${appendingInvoiceId}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 print:hidden">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                        <div className="p-5 border-b border-gray-200 flex justify-between items-center bg-gray-50 rounded-t-xl">
                            ...
                            <button onClick={() => setAppendingInvoiceId(null)} className="text-gray-400 hover:text-red-500 transition"><X size={20} /></button>
                        </div>
...
export default React.memo(DyeingSavedBillsTab);
