    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
                <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-700"><X size={16} /></button>
                <div className="flex items-center gap-3 mb-5">
                    <div className="w-10 h-10 rounded-xl bg-red-100 flex items-center justify-center"><Lock className="text-red-500" size={16} /></div>
                    <div>
                        <h2 className="font-black text-gray-900 text-base">Dyeing Ledger Close করুন</h2>
                ...
                <form onSubmit={(e) => { e.preventDefault(); if (!title.trim()) return toast.error("title দিন"); onConfirm(title.trim()); }} className="space-y-4">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button type="submit" disabled={loading || !title.trim()} className="flex-1 cursor-pointer px-4 py-2.5 rounded-xl bg-red-500 text-white text-sm font-black hover:bg-red-600 disabled:opacity-50 flex items-center justify-center gap-2">
                            {loading ? <span className="animate-spin border-2 border-white border-t-transparent rounded-full w-4 h-4" /> : <><Lock size={12} /> Confirm Close</>}
                        </button>
                    </div>
                </form>
            </div>
