...
/**
 * Manual transport order history edit form (Transport Management only).
 */
export default function EditTransportOrderModal({
  ...
  const updateTransportOrder = useMutation(api.transportOrders.update);
  ...
  useEffect(() => {
    if (order && open) {
      ...
      setForm({
        ...
        clotheType: order.clotheType || "",
        quality: order.quality || "",
        colour: order.colour || "",
        finishingType: order.finishingType || "",
        totalGoj: order.totalGoj != null ? String(order.totalGoj) : "",
        totalBundle: order.totalBundle != null ? String(order.totalBundle) : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Transport order history updated");
      onClose();
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Failed to update transport order";
      toast.error(message);
      console.error(err);
    } finally {
      setSubmitting(false);
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <motion.div
            ...
            role="dialog"
            aria-labelledby="edit-transport-order-title"
          >
            <div className="flex items-start justify-between gap-3 mb-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-[#3a6a9f]/10 flex items-center justify-center shrink-0">
                  <Edit size={22} className="text-[#3a6a9f]" />
                </div>
                <div>
                  <h3
                    id="edit-transport-order-title"
                  ...
                  <p className="text-[13px] text-[#26251e]/60 dark:text-[#f7f7f4]/60 mt-0.5">
                    Update the details for this transport trip
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 mt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg font-medium text-[14px] text-white bg-[#3a6a9f] hover:bg-[#2a4d74] shadow-md hover:shadow-lg transition-all disabled:opacity-60 disabled:hover:shadow-md"
                >
                  <Edit size={16} />
